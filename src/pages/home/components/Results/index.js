        ...
        <>
            {recommendedPhase && (
                <div className="font-josefin-sans flex justify-center">
                    ...
                    <p className="text-xl">Perform at the {recommendedPhase} moon.</p>
                    <p className="text-xl">Herbs: {recommendedHerbArray.map(each => `${each.name} `)}</p>
                    <p className="text-xl">Crystals: {recommendedCrystalArray.map(each => `${each.name} `)}</p>
                    </div>
                </div>
            )}
        </>
